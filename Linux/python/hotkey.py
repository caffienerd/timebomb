#!/usr/bin/env python3
#hotkey.py - Clean suppression system with udev hot-swap detection
...
# Try to import pyudev for hardware detection
try:
    import pyudev
    HAS_PYUDEV = True
except ImportError:
...
class HotkeyManager:
    def __init__(self, app_manager):
        ...
        self.devices = {}  # path -> (real_device, virtual_device) OR just real_device
        self.running = False
    ...
    def find_keyboards(self):
        """Find all keyboard devices (excluding our own virtual ones)"""
        keyboards = {}
        ...
        try:
            for path in evdev.list_devices():
                try:
                    ...
                    # Skip our own virtual devices to avoid loops
    ...
    def _setup_device(self, path, real_device):
        """Setup device - try with suppression first, fallback to listen-only"""
        with self.devices_lock:
            if self.suppression_enabled:
                try:
                    # Try to grab for suppression
                    real_device.grab()
                    print(f"[HOTKEY] ✓ Grabbed: {real_device.name}")
                    
                    # Create virtual device with safe name (max 80 chars)
                    safe_name = real_device.name[:50]  # Leave room for prefix
            ...
            # Fallback: listen-only mode (no grabbing)
            self.devices[path] = real_device
    
    def _cleanup_device(self, path):
        """Cleanup a device - FLUSH ALL KEYS FIRST"""
        with self.devices_lock:
            ...
            if isinstance(device_entry, tuple):
                # Suppression mode: flush, ungrab, close virtual
                ...
                # Release any tracked suppressed keys
        ...
        try:
            ...
            for device in iter(monitor.poll, None):
                ...
                if device.action == 'remove':
                    ...
                    # Find and cleanup matching devices
                    with self.devices_lock:
    ...
    def should_suppress(self, keycode):
        """Check if this key should be suppressed from OS"""
        # Only suppress TBKs when Win is pressed AND no other modifiers
        if keycode in self.combo_keys and self.win_key_pressed and not self.other_modifiers:
            return True
        return False
    
    def listen_worker(self):
        """Main listening thread with optional suppression"""
        while self.running:
            ...
            with self.devices_lock:
                ...
                # Get all devices (real or tuple of real+virtual)
                device_list = []
                path_map = {}  # device.path -> (path, virtual_device or None)
                ...
                for real_dev in r:
                    ...
                    try:
                        for event in real_dev.read():
                            ...
                            if event.type == ecodes.EV_KEY:
                                ...
                                if event.value == 1:
                                    ...
                                    if virtual_dev and self.should_suppress(event.code):
                                        ...
                                        # Inject dummy to prevent Win menu
                                    ...
                                    # Always handle for hotkey detection
                                    self.handle_key_press(event.code)
                                
                                # KEY RELEASE
                                elif event.value == 0:
                                    # Suppress release if we suppressed press
                            ...
                            # Pass through to OS if NOT suppressed
                            if not suppress and virtual_dev:
                                virtual_dev.write_event(event)
                                virtual_dev.syn()
                    
                    except OSError:
                        # Device disconnected - will be handled by udev
    ...
    def start(self):
        ...
        # Check if any devices have suppression
        ...
        # Start udev monitor thread (if available)
        if HAS_PYUDEV:
